...
const cases: RegressionCase[] = [
  {
    ...
    acceptance: [
      ...
      "When Resume persists compaction or startup work before echoing its activating prompt, the shared semantic order still renders prompt, Worked, then final answer.",
      "The order is unchanged after browser refresh and selecting the session from the sidebar.",
    ],
    evidence: [
      "packages/runtime-protocol/src/conversation-item-order.test.ts",
      "packages/runtime-daemon/src/conversation-projector.test.ts",
      "packages/client-web/src/conversation-feed.test.ts",
  ...
  {
    id: "NEW-TASK-DRAFT-OWNERSHIP-001",
    severity: "P0",
    providers: ["all"],
    automation: ["unit", "fake_browser"],
    title: "New task transfers its draft only after provider-bound startup accepts it",
    acceptance: [
      "New task clears its text and attachments before starting a Session and rejects duplicate submissions while startup is in flight.",
      "A text-only first question travels inside /sessions/start with stable message and turn identities; New task must not depend on a second /input request after navigation begins.",
      "The provider receives the exact question once and its Agent reply renders in Chat; neither Session creation nor daemon queue ownership alone is accepted as proof of delivery, and /sessions/start cannot succeed before provider acceptance.",
      "If provider-bound startup rejects, the original New task draft and attachments remain user-owned and are restored instead of disappearing.",
    ],
    evidence: [
      "packages/client-web/src/hooks/useWorkbenchComposerState.test.ts",
      "packages/client-web/src/hooks/useWorkbenchComposerState.ts",
      "packages/client-web/src/session-store-session-startup.test.ts",
      "packages/runtime-daemon/src/codex-live-client.test.ts",
      "scripts/native_codex_browser_smoke.py",
    ],
  },
  {
  ...
  },
  {
    ...
    acceptance: [
      ...
      "Every turn starts its Working / Worked / Interrupted row with a provider icon; Desktop hover and compact-layout tap reveal provider/model/effort without consuming a footer or standalone row.",
    ],
    evidence: [
      "packages/client-web/src/typography-contract.test.ts",
      "packages/client-web/src/components/chat/AssistantTurnModelMeta.tsx",
      "packages/client-web/src/conversation-feed.test.ts",
      "scripts/workspace_lifecycle_browser_smoke.py",
  ...
  },
  {
    ...
    acceptance: [
      ...
      "All responsive tiers share the same concise compatibility title/body, omit verbose generation identifiers, expose Mute today, and do not render Retry.",
  ...
  },
  {
    ...
    providers: ["all"],
    automation: ["unit", "fake_browser"],
    title: "Turn changed files use the unified Review surface",
    acceptance: [
      "Opening a Changed file from a turn card on every layout opens that turn's Review surface with the exact file preselected.",
      "The Review button, turn-file rows, and active Task summary share one modal path and never mutate Inspector visibility.",
      "Compact Review keeps its changed-file list collapsed above the diff until the user expands the dropdown.",
      "Closing Review returns directly to Chat with both Inspector panel states unchanged.",
    ],
    evidence: [
      ...
      "packages/client-web/src/App.tsx",
      "packages/client-web/src/components/chat/ChatThread.tsx",
      "packages/client-web/src/inspector/ReviewSurface.tsx",
      "scripts/workspace_lifecycle_browser_smoke.py",
    ],
  },
  {
    ...
    acceptance: [
      ...
      "Standalone PWA Chat is a single inset row while idle, expands from an explicit interaction state, grows the live textarea to a bounded cap during iOS IME input, and only then scrolls internally; permission/Plan/model controls remain interactive and their portal menus stay inside the visual viewport above the keyboard.",
    ...
    evidence: [
      "scripts/workspace_lifecycle_browser_smoke.py",
      "packages/client-web/src/composer-contract.test.ts",
      "packages/client-web/src/composer-popover-layout.test.ts",
      "packages/client-web/src/composer-popover-layout.ts",
      "packages/client-web/src/components/UnifiedComposerSurface.tsx",
      "packages/client-web/src/components/TokenizedTextarea.tsx",
      "packages/client-web/src/components/workbench/panes/NewSessionComposer.tsx",
  ...
  {
    id: "CHAT-VISUAL-OUTPUTS-001",
    severity: "P1",
    providers: ["all"],
    automation: ["unit", "fake_browser"],
    title: "Terminal visual outputs use a bounded lazy thumbnail gallery",
    acceptance: [
      "Provider-native image artifacts and conservative standalone local image links project through turn.outputs instead of extension-based anchor rewriting.",
      "Only terminal image outputs appear after the final answer; non-images remain in Inspector and Markdown-embedded images are not duplicated.",
      "The gallery shows at most eight compact wrapping thumbnails, validates actual local image signatures, and preserves a placeholder on failure.",
      "Desktop and PWA reuse the same gallery; opening and closing a PWA thumbnail returns directly to Chat without revealing Inspector.",
    ],
    evidence: [
      "packages/runtime-daemon/src/conversation-resource-projector.test.ts",
      "packages/runtime-daemon/src/codex-app-server-activity.test.ts",
      "packages/runtime-daemon/src/host-file-read.test.ts",
      "packages/client-web/src/conversation-visual-outputs.test.ts",
      "packages/client-web/src/components/chat/ConversationVisualOutputGallery.tsx",
      "scripts/workspace_lifecycle_browser_smoke.py",
    ],
  },
  {
  ...
  },
  {
    ...
    acceptance: [
      ...
      "After the reader detaches from the tail, lazy images or Markdown reflow inside one oversized row preserve the visible descendant's pixel position.",
  ...
  {
    id: "CANVAS-SESSION-DROP-001",
    severity: "P1",
    providers: ["all"],
    automation: ["unit", "fake_browser"],
    title: "Running and stopped sessions drag from Sidebar into Canvas panes",
    acceptance: [
      "Every visible Session title is a draggable source, including stopped history rows.",
      "Running rows carry a runtime target while stopped rows carry stable provider and providerSessionId identity.",
      "Canvas accepts the RAH MIME payload or its namespaced plain-text fallback and copies the target into the pane without removing the Sidebar row.",
      "Malformed payloads and ordinary dragged text are ignored.",
    ],
    evidence: [
      "packages/client-web/src/components/workbench/canvas/canvas-session-drag.test.ts",
      "packages/client-web/src/SessionSidebar.tsx",
      "packages/client-web/src/components/workbench/canvas/CanvasWorkbench.tsx",
      "scripts/workspace_lifecycle_browser_smoke.py",
    ],
  },
  {
    id: "CANVAS-PANE-FILE-VIEWER-001",
    severity: "P1",
    providers: ["all"],
    automation: ["unit", "fake_browser"],
    title: "Canvas local-file viewers stay pane-owned and adapt to pane size",
    acceptance: [
      "Each pane owns at most one local-file viewer and opening a file in one pane cannot replace another pane's viewer.",
      "Auto presentation uses the pane content box: panes at least 560x480 open a centered window, while smaller panes fill only their own pane.",
      "A pane window resizes vertically from its top or bottom edge between 280px and the pane bounds, without horizontal resize or movement.",
      "Clicking the current or another transcript file retargets and expands the same viewer from collapsed or windowed state while preserving explicit presentation and adjusted height.",
      "Maximize, restore, collapse, close, clear, and remove mutate only the owning pane; an explicit presentation choice survives later pane resize.",
      "The global Changed Files Review remains a separate single overlay and does not destroy pane-local viewers.",
    ],
    evidence: [
      "packages/client-web/src/components/workbench/canvas/CanvasPaneFileViewer.test.tsx",
      "packages/client-web/src/hooks/useCanvasController.test.ts",
      "packages/client-web/src/hooks/useCanvasController.ts",
      "packages/client-web/src/inspector/InspectorFileDetailDialog.tsx",
      "scripts/workspace_lifecycle_browser_smoke.py",
    ],
  },
  {
    id: "INSPECTION-CONTEXT-LIFECYCLE-001",
    severity: "P1",
    providers: ["all"],
    automation: ["unit", "manual"],
    title: "File and Review overlays close when their owning page context changes",
    acceptance: [
      "A Session file viewer stores the opening session, workspace root, and path instead of re-reading the current global selection.",
      "Navigating to another Session, Council, workspace, Canvas pane, or top-level Settings/Terminal context closes the old Session viewer and any Review not owned by the destination.",
      "Leaving Canvas or opening a top-level takeover clears every pane viewer, while merely activating another Canvas pane preserves each pane's independent viewer.",
      "Desktop and 390x844 layouts share the same lifecycle owner and never carry a stale file or Review over the new center page.",
    ],
    evidence: [
      "packages/client-web/src/hooks/useWorkbenchInspectionLifecycle.test.ts",
      "packages/client-web/src/hooks/useWorkbenchInspectionLifecycle.ts",
      "packages/client-web/src/inspector/ReviewOverlay.tsx",
      "packages/client-web/src/App.tsx",
      "docs/ui-regression-checklist.zh-CN.md",
    ],
  },
  {
    id: "HISTORY-HOT-CACHE-001",
    severity: "P0",
    providers: ["all"],
    automation: ["unit"],
    title: "A previously viewed conversation stays readable while its canonical tail catches up",
    acceptance: [
      "Returning to an already viewed Session renders the bounded in-memory page before a slow or unavailable daemon response.",
      "The cache is keyed by stable provider identity, is bounded by entry count, bytes, and TTL, and cannot create a Sidebar session absent from the authoritative catalog.",
      "A fresh daemon page replaces detached tail membership and establishes its own opaque cursor and revision; stale cursors never cross runtime identities, while bounded summary omissions cannot delete already rendered thinking from the same provider turn.",
      "Replay-gap recovery keeps readable baselines and refreshes only selected or otherwise visible conversations instead of flooding the remote link for every loaded Session.",
      "No transcript is persisted to localStorage or IndexedDB; after browser-process eviction or reload, the daemon revision-keyed hot cache remains the acceleration layer.",
    ],
    evidence: [
      "packages/client-web/src/session-conversation-memory-cache.test.ts",
      "packages/client-web/src/conversation.test.ts",
  ...
  },
  {
    ...
    providers: ["codex", "claude", "opencode"],
    automation: ["unit", "fake_browser"],
    title: "A stopped-session Send is atomically owned across slow live resume",
    acceptance: [
      "The first question, attachments, annotations, model, effort, and mode travel inside the same /sessions/resume request; the browser never relies on a post-resume /input request.",
      "The daemon canonical queue owns the stable client message id while startup is pending; /sessions/resume succeeds only after the provider accepts that exact input, and its returned summary no longer contains the submitting queue item.",
      "A slow large-history resume keeps the optimistic user message and Starting/Working state; a late provider idle notification cannot erase Working while turn/start or queued input is pending.",
      "An explicit live-resume failure remains an error and never masquerades as a successful read-only replay; turn rejection or uncertain transport leaves the question queued and visible after refresh.",
      ...
      "Both callers share one provider resume request and cannot create duplicate runtimes; if the in-flight resume already owns one question, every later question still gets its own queued delivery path.",
    ...
    evidence: [
      ...
      "packages/client-web/src/session-store-session-startup.ts",
      "packages/client-web/src/hooks/useWorkbenchComposerState.ts",
      "packages/runtime-daemon/src/session-startup-atomic.test.ts",
      "packages/runtime-daemon/src/codex-live-client.test.ts",
      "packages/runtime-daemon/src/codex-live-helpers.test.ts",
      "packages/runtime-daemon/src/codex-adapter.test.ts",
      "scripts/native_codex_browser_smoke.py",
    ],
  },
  {
