...
- RAH 不对 Claude tmux 维护权威 busy/idle。
- RAH 不在 Claude tmux 下维护隐藏消息队列。
...
Web chat 发送时，RAH 直接把文本写入 tmux 中的 Claude TUI 并提交。
...
Claude tmux 不应强行复用这套语义，否则会持续产生重复气泡、interrupt 漂移和 stop 卡住。

## 不变量

- Claude tmux Send 不进入 RAH hidden queue。
- Claude tmux Web Esc 不发布 synthetic `turn.canceled`。
- Claude tmux Chat 不依赖 RAH prompt state 排序。
