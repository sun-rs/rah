...
| --- | --- | --- |
| 创建/恢复 structured live session | `ProviderStructuredLifecycleAdapter.startSession/resumeSession` | Legacy/debug test surface。生产拒绝显式 `liveBackend: "structured"`；默认 Web/CLI live 不走 adapter lifecycle。 |
