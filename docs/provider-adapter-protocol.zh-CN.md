...
| 模型切换 | `setSessionModel(sessionId, request)` | 运行中切模型和 reasoning/variant。具体实现 adapter-owned。 |
...
```ts
setSessionModel(sessionId, { modelId, reasoningId? })
```
...
- `modes` 是否带正确 `role`。
- `startSession` / `resumeSession` 是否消费 `modeId/model/reasoningId`。
...
- 创建/claim 时只发送 `modeId/model/reasoningId`。
