...
- Council `compact` = 标题栏 + Council。
- Council `expanded` = 标题栏 + Council + Agents 侧边栏。

Canvas 的普通分屏 pane 只使用 `compact`，避免 2/3/4 分屏时侧栏挤压主内容；pane 最大化后切到 `expanded`，让用户获得接近完整 session / council 的工作体验。完整页面 shell 不是另一种对象 UI，而是把同一个对象主区域和侧边栏组合进顶层 workbench chrome。

Canvas pane 的持久化语义是固定槽位模型：

- Canvas 最多有 4 个固定 pane，内部 ID 固定为 `canvas-1` 到 `canvas-4`。
