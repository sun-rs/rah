...
旧的 `test:smoke:codex-browser`、`test:smoke:claude-browser`、`test:smoke:opencode-browser` 仍可作为 provider-specific smoke 辅助，但不替代 native local server probe 和当前 browser smoke。Gemini/Kimi CLI smoke 已删除，不属于默认 gate。
