...
当前 `main` 把 live 主线拆成 provider runtime：
...
- `rah codex/opencode`、Web New、Canvas New、Web Claim History 对 Codex/OpenCode 默认进入 native local server runtime；`rah claude` 和 Claude Web live 默认进入 zellij/TUI mux fallback。
...
| Structured test live | 只允许测试注入 adapter 直接调用 engine；公开 HTTP API 拒绝 `liveBackend: "structured"` | 可以 | 关闭 provider adapter client | injected adapter event + history |

Structured test live 的保留决策：

- 保留它作为内部测试 harness，而不是生产 live 主链路。
- 普通 daemon 不构造 Claude SDK/headless structured live adapter；该旧路径已删除。
- Codex/OpenCode 的 provider-server control adapter 是当前生产路径的一部分，虽然仍提供 structured event/control 能力，但默认 live backend 是 `native_local_server`。
...
| Structured test live | 关闭 injected provider adapter client，session 从 live list 消失 |
