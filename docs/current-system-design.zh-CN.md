...
| Legacy structured live | 旧测试/调试路径；公开 HTTP API 拒绝 `liveBackend: "structured"`，只允许测试注入 adapter 直接调用 engine | 可以 | 关闭 provider adapter client | provider SDK/API event + history |
