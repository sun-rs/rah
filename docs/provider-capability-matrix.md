...
- Claude uses the `tui_mux` / `tui_mux_fallback` runtime because it does not expose a stable Codex/OpenCode-style local app-server path for RAH. Its official TUI remains the work surface; provider history parsing is the structured Chat mirror.
