...
- 只有能继续 send/interrupt/archive 的 RAH live session 才算 RAH 管理写手。
...
如果某个 provider 只在追加瞬间打开文件、写完立即关闭 fd，那么没有 RAH runtime ownership 或 provider pid 元信息时，无法 100% 证明它仍在运行；RAH 会退回到“文件稳定 + 无活跃写手”的 closed-history 规则。
