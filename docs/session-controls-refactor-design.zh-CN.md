...
- Claude zellij/TUI fallback session 不假装支持运行中热切；运行中状态以 Claude 原生 TUI 为准。
...
Claude 当前 RAH runtime 是 `tui_mux_fallback` / zellij。拉起后真实 TUI 是 owner，RAH 不应假装能可靠热切所有内部模式。因此 Claude controls 默认只在 `prelaunch` 生效。
...
```json
{
  "error": {
    ...
    "message": "Claude zellij sessions can only apply approval mode before launch."
  }
}
```
...
- Web 前端不做后台定时刷新，也不在启动时预热所有 provider catalog。
- 刷新触发点只放在用户意图明确的位置：点击 provider 选择入口、打开 new/resume/live 的 Session Control、打开模型/权限菜单。
- catalog 使用短 TTL 缓存，当前实现为 5 分钟；TTL 内复用最后一次成功结果。
