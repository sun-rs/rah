...
显式 `liveBackend: "structured"` 仍保留旧 adapter control plane，用于测试、调试和少数增强场景。`RAH_LEGACY_WRAPPER=1 rah xxx` 也仍保留旧 wrapper handoff。它们的权限/模型能力大致如下，但不再是 PTY-first core 的承诺：
...
这个 legacy path 可以继续被测试覆盖，但新增产品能力应优先保证 native TUI PTY 主线，而不是扩大 structured live 适配面。
