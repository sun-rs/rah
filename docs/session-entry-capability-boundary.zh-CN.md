...
显式 `liveBackend: "structured"` 是旧 adapter control plane。生产禁用 live start/resume，只允许测试注入 adapter 时使用；`preferStoredReplay` 的只读历史 replay 不受此限制。旧 terminal wrapper handoff 代码仍作为内部 legacy 参考和 synthetic test surface 存在，但公开 `rah xxx` CLI 不再进入该路径。它们的权限/模型能力大致如下，但不再是 PTY-first core 的承诺：
