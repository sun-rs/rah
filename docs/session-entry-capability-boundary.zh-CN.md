...
旧的 “terminal handoff vs web-owned structured live” 仍作为 legacy/enhancement 参考保留，但不是当前主路径。
...
- `RAH_LEGACY_WRAPPER=1 rah xxx` 会进入旧 terminal wrapper handoff；该路径保留用于兼容，不是主链路。
...
显式 `liveBackend: "structured"` 是旧 adapter control plane。生产默认禁用 live start/resume，只有设置 `RAH_ENABLE_LEGACY_STRUCTURED_LIVE=1` 或测试注入 adapter 时才允许；`preferStoredReplay` 的只读历史 replay 不受此限制。`RAH_LEGACY_WRAPPER=1 rah xxx` 也仍保留旧 wrapper handoff。它们的权限/模型能力大致如下，但不再是 PTY-first core 的承诺：
