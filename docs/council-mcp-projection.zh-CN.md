...
Gemini CLI 的 Council 接入走 `tui_mux`，启动时 RAH 通过临时 `GEMINI_CLI_SYSTEM_SETTINGS_PATH` 注入 `rah_council` MCP server，避免改写用户 `~/.gemini/settings.json`，也不隔离用户登录态和原生 history。
