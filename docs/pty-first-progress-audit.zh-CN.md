...
- Legacy structured adapters still exist under `legacy-structured/`, but the normal daemon no longer constructs them and public HTTP rejects `liveBackend: "structured"`. They remain available only for direct injected tests. The remaining code gap is deciding whether to keep or delete this internal legacy/test surface entirely.
