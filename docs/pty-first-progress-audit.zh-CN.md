...
- Legacy structured adapters still exist. The structured live clients are now path-isolated under `legacy-structured/`, and they are no longer default. Claude and Gemini now have separate stored-history adapters, but Codex/Kimi/OpenCode provider adapter classes still contain both stored-history/mirror logic and explicit structured fallback plumbing. The remaining code gap is applying the same split to the other providers.
