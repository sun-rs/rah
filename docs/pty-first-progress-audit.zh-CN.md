...
| Stored replay is independent from legacy structured live | `ProviderStoredHistoryAdapter.resumeStoredSession`; five stored-history adapters create read-only replay sessions; `RuntimeEngine.resumeStoredReplaySession()` intercepts `preferStoredReplay`; read-only replay close no longer requires a structured lifecycle adapter | Done |
...
| Legacy structured path no longer default | Web, CLI, and default daemon RuntimeEngine prefer native TUI; default provider adapters are stored-history/debug only and no longer construct legacy structured adapters | Done |
...
| Default legacy structured construction removed | `createDefaultProviderAdapters()` now registers `DebugAdapter` plus five stored-history adapters only; five `legacy-structured/*-structured-adapter.ts` implementations remain direct injected-test surface only; public HTTP rejects `liveBackend: "structured"` | Done |
...
- `npm run test:provider-contracts`: 135 pass
- `npm run test:runtime`: 378 pass after closing the default wrapper-control WebSocket route, disabling the wrapper runtime by default, and rejecting legacy structured liveBackend at the public HTTP boundary
...
`npm run test:smoke:wrapper` remains available as an explicit legacy/internal smoke for the old wrapper-control path, but it now uses a dedicated test daemon with wrapper-control and the wrapper runtime enabled and is no longer part of the PTY-first native gate. The public `rah <provider>` CLI path now enters the native TUI runtime directly, and the normal daemon keeps `/api/wrapper-control` closed and the wrapper runtime disabled by default.
...
This guard verifies that production `RuntimeEngine` rejects explicit `liveBackend: "structured"` live start/resume. The public HTTP request parser also rejects `liveBackend: "structured"` before engine dispatch, so the path remains available only for injected test adapters calling the engine directly; `preferStoredReplay` read-only history replay is not blocked.
...
- `node --import tsx --test --test-force-exit packages/runtime-daemon/src/http-server.test.ts packages/runtime-daemon/src/runtime-engine.test.ts packages/runtime-daemon/src/runtime-terminal-capabilities.test.ts packages/runtime-daemon/src/terminal-wrapper-control.test.ts`: 73 pass
...
- Legacy structured adapters still exist under `legacy-structured/`, but the normal daemon no longer constructs them and public HTTP rejects `liveBackend: "structured"`. Decision: keep this as an internal injected-test/compatibility harness only, not as a production live path.
- Legacy wrapper protocol/runtime still exists for synthetic tests and internal legacy reference. The five old provider-specific wrapper programs are deleted; the public `rah xxx` CLI no longer exposes an env escape hatch into wrapper handoff, the normal daemon does not expose `/api/wrapper-control`, and RuntimeEngine does not enable the wrapper runtime unless an explicit test daemon opts in. Decision: keep the small wrapper registry/protocol harness for regression tests only, not as a third public live system.
