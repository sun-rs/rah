...
| `GLOBAL-NOTICE-MUTE-001` | 跨设备静音 | 同一隔离 daemon 的两个不同 origin/client 中，任一端 `Mute today` 后另一端重新聚焦或在 10 秒同步周期内隐藏；reload/PWA standalone 保留，daemon 主机下一本地午夜失效；普通 workbench/session/auth/transport error 不受影响 |
| `PWA-TURN-CHANGE-PREVIEW-001` | 本轮文件统一审查 | 所有布局中，回复卡片的单文件入口、审查按钮与 Task 明细的 `Changed files` 共用同一个不修改 Inspector 状态的 Review 路径；单文件入口预选精确文件；PWA 将文件列表收纳为 diff 上方默认折叠的下拉区；关闭后直接回到 Chat，Desktop 也不得再打开或维护本轮专属右侧栏 |
| `CANVAS-PANE-FILE-VIEWER-001` | pane 文件预览隔离与自适应 | A/B pane 分别点击回复文件后各自出现被 pane 边界裁剪的 viewer，B 的请求和迟到响应不改写 A；pane `>=560×480px` 默认靠右浮窗并保留左侧正文点击带，较小 pane/PWA 默认只占满所属 pane；窗口化只允许上/下边缘把高度调到 `280px` 与 pane 边界之间；显式最大化/还原、折叠、关闭、clear/remove 按 pane 隔离；折叠态或窗口态点击当前/其他正文文件都以新 request identity 激活同一 viewer、解除折叠并 retarget，且保留窗口高度；全局 Changed Files Review 始终唯一且打开/关闭不销毁 pane viewer |
| `INSPECTION-CONTEXT-LIFECYCLE-001` | 文件/审查窗口页面归属 | 普通 Session 文件 viewer 保存来源 session/workspace/path；切换 Session、Council、Canvas、workspace 或打开 Settings/Terminal 后，普通 viewer 与旧 owner Review 在绘制前关闭，不得跟随 selection 偷换读取目标；离开 Canvas/顶层功能接管时清空所有 pane viewer，Canvas 内仅切换 active pane 仍保留各 pane 独立 viewer |
