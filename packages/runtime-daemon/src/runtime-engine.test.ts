  ...
  test("native TUI backend pre-binds provider session ids for providers that support it", async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "    JSON.stringify({ type: 'assistant', uuid: 'claude-native-assistant', cwd: process.cwd(), sessionId, timestamp: now, message: { content: [{ type: 'text', text: 'Claude native answer' }], stop_reason: 'end_turn' } }),",
        "    JSON.stringify({ type: 'system', subtype: 'turn_duration', uuid: 'claude-native-duration', cwd: process.cwd(), sessionId, timestamp: now, durationMs: 1 }),",
        "  ].join('\\n') + '\\n');",
        "}",
