...
export async function createCodexStdioAppServerClient(binary?: string): Promise<CodexJsonRpcClient> {
  ...
  const child = spawn(resolvedBinary, ["app-server"], {
    ...
    env: providerChildEnv(rahNativeServerEnv("codex")),
  });
...
export function shouldUseCodexWebSocketTransport(
  configuredTransport = process.env.RAH_CODEX_APP_SERVER_TRANSPORT,
): boolean {
