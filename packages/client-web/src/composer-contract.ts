...
export const EMPTY_STATE_COMPOSER_LAYOUT = {
  textareaClassName:
    `${COMPOSER_TEXTAREA_BASE_CLASS_NAME} rounded-2xl ${COMPOSER_TEXT_PADDING_CLASS_NAME} pr-16 pb-24 md:pr-[4.5rem] md:pb-[4.5rem] min-h-[120px]`,
  textareaContentClassName:
    `${COMPOSER_TEXT_PADDING_CLASS_NAME} pr-16 pb-24 md:pr-[4.5rem] md:pb-[4.5rem] ${COMPOSER_TEXT_BASE_CLASS_NAME} min-h-[120px]`,
  roundSecondaryButtonClassName:
  ...
  controlsRowClassName:
    "absolute bottom-3 left-3 right-3 z-10 flex items-end justify-between gap-1.5 md:gap-2",
  leftControlsClassName: "flex min-w-0 flex-1 flex-wrap items-end gap-1 md:gap-2",
  workspaceTriggerClassName:
    "inline-flex h-8 md:h-9 w-[4.5rem] md:w-[9rem] min-w-0 items-center gap-1.5 rounded-full border border-[var(--app-border)] bg-[var(--app-bg)]/90 px-2 text-[11px] text-[var(--app-fg)] transition-colors hover:bg-[var(--app-subtle-bg)]",
} as const;
