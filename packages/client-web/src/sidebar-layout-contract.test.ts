...
describe("sidebar layout contract", () => {
  test("locks fixed action slots and row heights", () => {
    ...
    assert.match(SIDEBAR_LAYOUT.sessionPinSlotClassName, /\bh-8\b/);
    assert.match(SIDEBAR_LAYOUT.sessionPinSlotClassName, /\bw-8\b/);
  ...
  test("locks sidebar indentation and meta width tokens", () => {
    ...
    assert.match(SIDEBAR_LAYOUT.sessionStatusBadgeBaseClassName, /text-\[11px\]/);
  });
